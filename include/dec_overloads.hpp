...
auto operator+(const FormExpression<Traits1,Expr1> & a, const FormExpression<Traits2,Expr2> & b)
    ->
    const FormExpression<typename mtao::deduceFormAdditionTraits<Traits1,Traits2>::type,
    decltype(
            std::declval<Expr1>()+std::declval<Expr2>()
            )>
{ 
    return
        FormExpression<typename mtao::deduceFormAdditionTraits<Traits1,Traits2>::type,
        decltype(
                std::declval<Expr1>()+std::declval<Expr2>()
                )> {a.expr+b.expr};
}
...
auto operator*(const FormExpression<Traits1,Expr1> & a, const FormExpression<Traits2,Expr2> & b)
    ->
    const FormExpression<typename mtao::deduceFormCompositionTraits<Traits1,Traits2>::type,
    decltype(
            std::declval<Expr1>()*std::declval<Expr2>()
            )>
{ 
    return
        FormExpression<typename mtao::deduceFormCompositionTraits<Traits1,Traits2>::type,
        decltype(
                std::declval<Expr1>()*std::declval<Expr2>()
                )> {a.expr*b.expr};
}
